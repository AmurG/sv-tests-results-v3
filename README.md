...
|---|---|---|---|
| `circt_verilog_extended` | **compile DEFAULT arm** (no `--four-valued`) | **4999/5093** | 169/169 |
| `Arcilator_extended` | **THE DEFAULT ARM** — two-valued; what a default invocation gets | **4664/5092** — BOARD ≡ HONEST | **166/169** |
| `circt_verilog_extended_4state` | **OPT-IN** `--four-valued` compile arm | **5038/5093** (rendered; the renderer note below) | 169/169 |
| `Arcilator_extended_4state` | **OPT-IN four-valued arm** (the arm the v1/v2 pages headline) | **4877/5092** — BOARD ≡ HONEST | 162/169 |

The `uvm_id_register` red inside both simulate columns' `uvm` cells is proven by reference identity, re-verified at this publish: our transcript is line-for-line identical to the second reference's, including every timestamp and all 26 UVM_ERRORs — the example's `create_map(..., UVM_BIG_ENDIAN)` makes `uvm_reg_map` byte-swap the full 64-bit `uvm_reg_data_t` inside the UVM library itself, and no conforming IEEE 1800.2 simulator can pass this row as graded (full disclosure on the [v2 page](https://amurg.github.io/sv-tests-results-v2/)). The `_4state` uvm cells are UNCHANGED at this publish and still read -2 vs the 2026-08-23 page: the reopened `property/sequence_local_var_test_uvm` pair (the grading bullet below).
...
- **STANDING SINCE THE EARLIER 2026-08-25 PUBLISH — two more verdict-scanner holes closed there, the give-back fully named; re-verified here (all give-back rows stay red).** The strict evaluator had two blind spots, repaired in the GRADER, not the engine: a bare all-caps `FAIL` verdict token was invisible to the verdict scan (21 ivtest sources print exactly that), and the runtime-output scan applied only to `.sv` sources, so a silenced `.v`-source simulation (rc=0, no output at all) kept its green. Validated old-vs-new before any board ran (banked arms reproduce exactly under the old grader; the new grader's flip set equals an independently surveyed prediction exactly; doctored controls bite). Give-back on THESE boards: 10 named rows on the DEFAULT simulate column, the same 10 plus `regress-vlg_pr1676071_iv` on the opt-in simulate column, zero on either compile column — every row named on the [v2 page](https://amurg.github.io/sv-tests-results-v2/). Also standing from that publish, REOPENED honestly (both rows still red here): `property_local_var_test_uvm` and `sequence_local_var_test_uvm` on the opt-in simulate column — their prior green was masked by a change violating IEEE 1800-2023 4.5's region ordering, and that change is excluded from this build for exactly that violation; the proper cure (moving the NBA commit point to the LRM's NBA region) is tracked.

Movement against the previous (earlier 2026-08-25) v3 publish, per column. Every column
moves, every flip is named, and GREEN-TO-RED IS ZERO ON ALL FOUR. The compile columns:
`circt_verilog_extended` **+1** (`fx68k` — a HARNESS repair, below) and
`circt_verilog_extended_4state` **+10** (`fx68k` plus the 9-row switch-level primitive class
— this wave's frontend-import feature work necessarily moves the elaboration-only compile
arm; the DEFAULT compile invocation keeps all 9 rows red symmetrically; two of the 9 —
`regress-vlg_br_gh99u_iv`, an honest engine red that now SIMULATES and prints its own
failing self-check, and `regress-vlg_test_mos_strength_reduction_iv`, a GRADER-LENS red
whose simulation prints only PASSED while an engine diagnostic in the same log quotes
`FAILED:`-shaped IR string literals the whole-log verdict scan trips on — stay red on the
simulate columns). The simulate columns: `Arcilator_extended` (the DEFAULT arm) reads
**4664/5092**, +1 = `fx68k` alone — its ENGINE verdicts did not move at all, so **this
wave's engine work is again invisible to a default invocation**; `Arcilator_extended_4state`
(the opt-in arm) reads **4877/5092** on a **13 red-to-green / ZERO green-to-red ledger with
every gain named**: **5** are the power/arithmetic unknown-operand value-semantics family
(`regress-vlg_arithunknown_iv`, `regress-vlg_powca_iv`, `regress-vlg_powproc_iv`,
`regress-vlg_pr2909386a_iv`, `regress-vlg_pr2909386b_iv` — each prior log printed its own
failing self-check; the cures are the zero-to-a-negative-power and unknown-operand rules
of IEEE 1800-2023 Table 11-4 and kin, plus a subtraction-fold guard; `powca`/`powproc`
are 2026-08-22 case-leak give-back rows honestly re-earned), **7** are the switch-level
primitive class (`regress-sv_br_gh1223c_iv`, `regress-vlg_br_gh99k_iv`, `regress-vlg_cmos_iv`,
`regress-vlg_rnpmos_iv`, `regress-vlg_rnpmos2_iv`, `regress-vlg_test_rnmos_iv`,
`regress-vlg_test_rpmos_iv` — the rnmos/rpmos/cmos/rcmos resistive and complementary MOS
primitives land this wave; each prior log refused the primitive at import), and **1** is
`fx68k` — a HARNESS repair, not an engine gain: the prior boards' invocation failed at
elaboration (`'fx68kTop' is not a valid top-level module`) because the harness omitted the
compile-time define (`FX68K_TEST`) gating this core's testbench top and dropped a
duplicate-initial-driver compatibility flag the engine implements; the harness now passes
both, the row flips on ALL FOUR columns, and no page books it as engine movement. The
engine-attributable step on the opt-in simulate column is **+12**. A RIG CORRECTION rides
this publish: the harness timeout override now acts as a FLOOR of each row's declared
budget (max(declared, 900)) instead of clipping it — zero rows moved on that account
(the two `bp_multicore_4*` giants ran to their full declared 1000 s budgets, 1002.5 s and
1002.6 s measured, and stay rc=71-red as the register-retention price; an engine-side
state-lowering fix removed a quadratic diagnostic-printing hot spot on this family —
383× on the largest configuration, output-certified byte-identical — and the rows remain
simulation-bound past their budgets even so). The earlier 2026-08-25 publish's 18 gains
all stay green and its 19 named give-backs all stay red (causes standing, its bullets
above and the v2 page's ledger).
...
**Source disclosure — PUBLICATION POSTURE (standing, by ruling): the dashboards are public; the engine source is NOT published.** The four extended columns were built from the campaign's engine tree of 2026-08-25; results provenance — the exact source tree, build receipts and board banks — is retained privately, every number is reproducible from that retained record, and nothing on this page implies the source is available (each row's `circt_ref` reads `not published`; the earlier revisions' public snapshot branches are retired). Two statements, both measured:
...
`Arcilator_extended_4state` 4564 → 4777 → 4819 → 4829 → 4865 → 4864 → **4877** (this publish net **+13**: 12 engine gains plus the `fx68k` harness repair, ZERO give-backs — the ledger above; the DEFAULT arm's engine verdicts did not move at all). The 2026-08-15 publish's
...
`assert_final_test_uvm`, green on the DEFAULT column beside them), and — since the earlier 2026-08-25 publish — the reopened `property/sequence_local_var_test_uvm` pair (the scanner-and-reopening bullet above; also green on the DEFAULT column). On the opt-in arm the
...
| simulate (the OPT-IN four-valued arm vs the DEFAULT two-valued arm, page lens) | 18 | **22** | +123 | **+235** |
...
`FAIL` text that demoted it). On the simulate lens the OPT-IN four-valued arm sits **213 rows
above** the DEFAULT arm, where at the earlier 2026-08-25 publish it sat 201 above, at
2026-08-23 192, at 2026-08-22 156, at 2026-08-19 147, at 2026-08-15 105, and at
2026-08-06 it sat 76 below. The 22 simulate
...
at the earlier 2026-08-25 publish), the 2 budget giants BACK in the set at rc=71 (the register-retention price),
...
on both compile columns (rendered and strict agree at 5038 on the four-valued compile column,
...
strict-graded counts are 5038 (`circt_verilog_extended_4state`), 4665 (`Arcilator_extended`)
and 4878 (`Arcilator_extended_4state`), while `circt_verilog_extended` has no seam and is
4999 either way.
...
measured log-shape changes are: (a) the value-content changes of the 12 named engine gains
(the power/arithmetic family now prints its self-check passes; the switch-primitive rows
now simulate at all); (b) the changed `fx68k` harness invocation (the define and the
un-dropped flag on its command line); the earlier 2026-08-25 publish's scanner demotions
and four-valued-aggregate warnings remain in the carried logs. The timing-annotation
...
simulate columns (4877 vs 4878 / 4664 vs 4665, exactly `basic-tagged-union`) is the renderer-lens
